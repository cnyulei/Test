	...
	for(int i=0;i<ptn_len+1;i++)
	{
		...
		for(j=0;j<head_level[i].count+1;j++)
		{
			...
			*/
			...
			tmp->up_downpath=0;
			for(int k=tmp->deltadistance;k<d+1;k++)
			{
				//做乘法
				for(int m=tmp->deltadistance;m<d+1+tmp->deltadistance-k;m++)
					tmp->up_downpath+=((tmp->uppath)[k])*((tmp->downpath)[m]);
			}
			tmp=tmp->next;
		}
	}
...
void cal_nodeinpath(int *nodeinpath,bool *used)
{
	...
	if(d==0)
	{
		...
		for(i=0;i<ptn_len+1;i++)
		{
			...
			for(j=0;j<=head_level[i].count;j++)
			{
				...
				nodeinpath[nodename]+=tmp->uppath[0]*tmp->downpath[0];
				tmp=tmp->next;
			}
		}
	}
	else
	{
		for(i=0;i<ptn_len+1;i++)
		{
			...
			for(j=0;j<head_level[i].count+1;j++)
			{
				nodeinpath[tmp->position]+=(tmp->up_downpath);  //每个结点的树根树叶路径数之和，此处应该为满足伽马距离的树根树叶路径数
				tmp=tmp->next;
			}
		}
	}
